...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect(getCharacter('ARYA'.toLowerCase())).toBeTruthy();
    expect(getCharacter('BRIENNE'.toLowerCase())).toBeTruthy();
    expect(getCharacter('MELISSANDRE'.toLowerCase())).toBeTruthy();
    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    const nomeIndefinido = () => {
      if (getCharacter() !== 'arya' && getCharacter() !== 'brienne' && getCharacter() !== 'melissandre' ) {
        return undefined;
      }
    }
    expect(nomeIndefinido()).toBeUndefined();
  });
});
